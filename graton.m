...
columns.subject.col_num = find(ismember(columns.subject.name,header));
columns.subject.conditions = unique(data(:,columns.subject.col_num));
%ACC
columns.acc.name = input('Accurace Col: ','s');
columns.acc.col_num = find(ismember(columns.acc.name,header));
%RT
columns.RT.name = input('RT Col: ','s');
columns.RT.col_num = find(ismember(columns.RT.name,header));
%BLOCK
columns.block.name = input('RT Col: ','s');
columns.block.col_num = find(ismember(columns.block.name,header));
...
if strcmp(input('within / between subjects Design?: ','s'),'within')
    ...
    columns.condition.col_num = find(ismember(columns.condition.name,header));
...
for i = 1:length(vars)
    ...
    columns.vars(i).col_num = find(ismember(columns.vars(i).name,header));
...
if strcmp(input('handed response?: ','s'),'yes')
    ...
    columns.handed_resp.col_num = find(ismember(columns.handed_resp.name,header));
    columns.handed_resp.conditions = unique(data(:,columns.handed_resp.col_num));

end

columns.congruity.name = input('Congruity Col: ','s');
columns.congruity.col_num = find(ismember(columns.congruity.name,header))
